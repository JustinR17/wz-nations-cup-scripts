...
PLAYER_COUNT_TO_LINK_COLUMN = {1: "F", 2: "J"}
...
class CreateGames:
    ...
    def parse_sheet_matchups(self, round):
        with open(
            f"data/matchups_output_r{round}.json", "r", encoding="utf-8"
        ) as json_file:
    ...
    def create_games(
        self, matchups: List[Matchup], round: int, template: str
    ) -> List[Matchup]:
        for matchup in matchups:
            log_message(
                f"Beginning game creation for {matchup.teams[0].name} vs. {matchup.teams[1].name}",
                "create_games",
            )

            for game in matchup.games:

                title = f"Nations' Cup 2024 R{round} {matchup.teams[0].name} vs. {matchup.teams[1].name}"
                description = f"""This game is a part of the Nations' Cup R{round}, run by Rento. You have 3 days to join the game.
...
"""
                ...
                try:
                    game_link = self.api.create_game(
                        [
                            (game.players[0].id, matchup.teams[0].name),
                            (game.players[1].id, matchup.teams[1].name),
                        ],
                        template,
                        title,
                        description,
    ...
    def write_games(self, sheet_name: str, matchups: List[Matchup], round: str):
        ...
        with open(
            f"data/games_output_r{round}.json", "w", encoding="utf-8"
        ) as output_file:
            log_message(
        ...
        for row in sheet_data:
            if not row:
                ...
                for matchup in matchups:
                    if (
                        row[0].strip() == matchup.teams[0].name
                        or row[0].strip() == matchup.teams[1].name
                    ):
                ...
                for game in current_matchup.games:
                    if (
                        row[1].strip() == game.players[0].id
                        and row[4].strip() == game.players[1].id
                        or row[1].strip() == game.players[1].id
                        and row[4].strip() == game.players[0].id
                    ):
        ...
        self.sheet.update_rows_raw(
            f"{sheet_name}!F1:F{len(updated_game_links)}", updated_game_links
        )
        log_message(
            f"Updated the google sheet with {len(updated_game_links)} new links",
            "write_games",
        )
...
"""
    ...
    def patch_broken_2v2_games(
        self, sheet_name: str, matchups: List[Matchup], round: int, template: str
    ) -> List[Matchup]:
        # Updating the sheet involves matching the matchups object to the sheet rows
            ...
            if not row or not row[0]:
                ...
                for game in current_matchup.games:
                    ...
                    for i in range(num_players):
                        if row[2 * i + 1] != str(sorted_players[i].id) or row[
                            2 * num_players + 2 * i + 2
                        ] != str(sorted_players[num_players + i].id):
                            does_game_match_row = False
                            break

                    # if game does match, add the link and remove the game from the matchup
                    if does_game_match_row:
                        self.create_individual_game(
                            game, current_matchup, template, round
                        )
                        while len(row) < 10:
                            row.append("")
        ...
        log_message(
            f"Updated the google sheet with {len(sheet_data)} new links",
            "patch_broken_2v2_games",
        )
