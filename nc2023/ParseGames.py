...
class ParseGames:
            ...
            for i, row in enumerate(tab_rows):
                ...
                if not row:
                    ...
                    if not team_a and not team_b:
                        ...
                        game.players[0].score, game.players[1].score = int(
                            score_row[1]
                        ), int(score_row[4])
                        ...
                        if game.outcome == Game.Outcome.FINISHED:
                            ...
                            if game.players[0].outcome == WarzonePlayer.Outcome.WON:
                                ...
                                score_row[1 if left_team_won else 4] = (
                                    int(score_row[1 if left_team_won else 4]) + 1
                                )
    ...
    def score_2v2_game(
        ...
        player_standings: Dict[str, RoundResult],
        newly_finished_games: Dict[str, List[WarzoneGame]],
        games_to_delete: Dict[str, List[WarzoneGame]],
    ):
        newly_finished_games_count = 0
        ...
        if game.outcome == Game.Outcome.FINISHED:
            ...
            if any(
                ...
                score_row[2 * len(sorted_players[0]) + 3] = (
                    int(score_row[2 * len(sorted_players[0]) + 3]) + 1
                )
                for player in sorted_players[1]:
                    player.score += 1
                ...
                row[2 * len(sorted_players[0])] = (
                    "defeats" if left_team_won else "loses to"
                )
                score_row[2 if left_team_won else 2 * len(sorted_players[0]) + 3] = (
                    int(
                        score_row[
                            2 if left_team_won else 2 * len(sorted_players[0]) + 3
                        ]
                    )
                    + 1
                )
            ...
            timezone.utc
        ) - game.start_time > timedelta(days=4):
            # Game has been in the join lobby for too long. Game will be deleted and appropriate winner selected according to algorithm:
            # 1. Assign win to left player if they have joined, or are invited and the right player declined
            # 2. Assign win to the right player if they have joined, or are invited and the left player declined
            # 3. Randomly assign win if both players are invited, or declined
            log_message(
                f"New game pased join time: {', '.join(map(lambda e: e.get_player_state_str(), game.players))} -- {game.link}",
                "update_new_games",
            )
            log_message(f"Storing end response: {game}")
            newly_finished_games.setdefault(tab, []).append(game)
            ...
            row[10] = "Deleted"
            if all(
                player.outcome == WarzonePlayer.Outcome.PLAYING
                for player in sorted_players[0]
            ) or (
                all(
                    player.outcome != WarzonePlayer.Outcome.DECLINED
                    for player in sorted_players[0]
                )
                and any(
                    player.outcome == WarzonePlayer.Outcome.DECLINED
